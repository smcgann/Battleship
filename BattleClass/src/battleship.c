...
void putShipOnGameBoard (Cell gameBoard[][COLS], WaterCraft ship,
	                     Coordinate_t * position, int direction) {
	int i = ship.length - 1;
...
void randomlyPlaceShipsOnGameBoard (Cell gameBoard[][COLS], WaterCraft ship[]) {
	Coordinate_t * position = (Coordinate_t *)malloc(sizeof(Coordinate_t));
	int direction = -1;
	int i = 0;
...
Boolean checkSunkShip (short sunkShip[][NUM_OF_SHIPS], short player, char shipSymbol, FILE *stream) {
	...
	puts("in checkSunkShip");fflush(stdout);
 ...
 * Post-condition: n/a
 */
Boolean isValidLocation (Cell gameBoard[][COLS], Coordinate_t * position,
...
Coordinate_t * getTarget (void) {
	Coordinate_t * target =(Coordinate *)malloc(sizeof(Coordinate));
    puts("in getTarget");fflush(stdout);
	fflush (stdin);
...
short checkShot (Cell gameBoard[][COLS], Coordinate_t * target) {
	puts("in checkShot");fflush(stdout);
	int hit = -2;
